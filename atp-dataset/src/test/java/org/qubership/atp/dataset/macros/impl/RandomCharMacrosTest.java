...
public class RandomCharMacrosTest extends AbstractMacrosTest {

    @Test
    public void testGenerationRandomCharactersWithLowerCase() throws Exception {
        RandomCharMacros charMacros = new RandomCharMacros();
        for (int i = 1; i < 100; i++) {
            String evaluate = charMacros.evaluate(Integer.toString(i), null);
            assertThat(evaluate, matchesToRegExp("[a-z]+"));
        }
    }

    @Test
    public void testEvaluateMacrosFromEvaluator() throws Exception {
        String evaluate = evaluator.evaluate("#CHARS(10)", new DataSetListImpl(), new DataSetImpl());
        assertThat(evaluate, matchesToRegExp("[a-z]+"));
    }
}
